...
{{--
Halaman Manajemen Anggaran (Budget)
Fitur:
- Menambah kategori anggaran baru (Modal Popup).
- Menampilkan daftar kategori dan progress penggunaan anggaran (Progress Bar).
- Menghitung persentase penggunaan berdasarkan data dari backend.
--}}
...
@section('content')
    <div x-data="{ open:false }" class="mb-6">
        <div class="flex items-center justify-between mb-6">
            ...
            <button @click="open = !open" type="button" class="px-5 py-2 bg-yellow-400 text-white rounded-full">+ Add
                Categories</button>
        </div>
            ...
            <form method="POST" action="{{ route('categories.store') }}" class="grid grid-cols-1 gap-4">
                ...
                <div class="flex gap-3">
                    ...
                    <button type="button" @click="open=false"
                        class="px-4 py-2 bg-gray-100 text-gray-700 rounded-full">Cancel</button>
                </div>
            </form>
        </div>
    </div>
    <p class="font-semibold mb-2">Categories</p>
    {{-- Loop item budget yang sudah dihitung di Controller --}}
