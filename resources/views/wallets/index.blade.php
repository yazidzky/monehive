...
{{--
Halaman Daftar Dompet (Wallets)
Menampilkan kartu-kartu dompet yang dimiliki user beserta saldonya.
Fitur:
- Pewarnaan otomatis kartu berdasarkan nama bank/e-wallet (Logic PHP internal view).
- Form tambah dompet baru langsung di halaman ini.
--}}
...
@section('content')
    ...
    @php
        // Logika pemilihan warna kartu dompet berdasarkan nama bank/ewallet
        $palette = [
            'bca' => 'bg-blue-500',
    ...
    <div class="flex gap-4 flex-wrap">
        ...
        <div class="bg-gray-200 rounded-2xl px-6 py-4 shadow">
            <form method="POST" action="{{ route('wallets.store') }}" class="space-y-3">
                ...
                <input type="text" name="name" placeholder="Nama Dompet..." class="w-56 border p-2 rounded bg-gray-50"
                    required>
                <input type="number" name="balance" placeholder="Nominal Awal..." class="w-56 border p-2 rounded bg-gray-50"
                    required>
